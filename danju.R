...
SM <- danju$SM
m22 <- lm(SM ~ site)
m23 <- lmer(SM ~ site + (1|depth), REML = F)
m24 <- lmer(SM ~ site + (1|site:depth), REML = F)

# Compare models
anova(m23, m24, m22) # m23 is the best model
# -> proceed with m23
m23 <- lmer(SM ~ site + (1|depth), REML = T)

# Test model fit
plot(m23) # quite homogenous

par(mfrow = c(1,2), mar = c(2,2,2,1))

hist(resid(m23)) # residuals are quite normally distributed
qqnorm(resid(m23))
qqline(resid(m23))
...
# Interpret model
Anova(m23, type = 2) # Type II sum of squares test
# Response: SM
#       Chisq Df Pr(>Chisq)    
# site 37.459  3   3.68e-08 ***

confint(m23) # pairwise contrasts
# Bamboo vs Manta; p < 0.05 *
# Bamboo vs. Moray; p > 0.05
# Bamboo vs. Tabon; p < 0.05 *
...
# Build models
m25 <- lm(S ~ site)
m26 <- lmer(S ~ site + (1|depth), REML = F)
m27 <- lmer(S ~ site + (1|site:depth), REML = F)

# Compare models
anova(m27, m26, m25) # models are not different
# -> proceed with m25
...
# Build models
m28 <- lm(E ~ site)
m29 <- lmer(E ~ site + (1|depth), REML = F)
m30 <- lmer(E ~ site + (1|site:depth), REML = F)

# Compare models
anova(m29, m30, m28) # models are not different
# -> proceed with m28

# Test model fit
par(mfrow = c(2,2), mar = c(2,2,2,1))
plot(m28) # quite homogenous

par(mfrow = c(1,2), mar = c(2,2,2,1))

hist(resid(m28)) # residuals are quite normally distributed
qqnorm(resid(m28))
qqline(resid(m28))
...
# Interpret model
Anova(m28, type = 2) # Type II sum of squares test
# Response: E
#           Sum Sq Df F value    Pr(>F)    
# site      0.95375  3  22.052 1.558e-07 ***
# Residuals 0.40368 28     

summary(m28) # pairwise contrasts
# Bamboo vs Manta; p < 0.001 ***
# Bamboo vs. Moray; p < 0.001 ***
...
SMstat <- describeBy(SM, site, mat = T, digits = 3)
...
SMstat$group1 <- factor(SMstat$group1, levels = c("Manta", "Moray", "Tabon", "Bamboo"))
...
# Submassive coral
SMplot <- ggplot(SMstat, mapping = aes(group1, mean)) +
              geom_col(width = 0.7, fill = c("#ef8407", "#7fb8d5", "#467289", "#f7b060")) +
              geom_errorbar(mapping = aes(ymin = mean - se, ymax = mean + se),
                            width = .1, lwd = .4) +
              geom_text(aes(x = group1, y = mean + se + 1.8,
